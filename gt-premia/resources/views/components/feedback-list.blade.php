<div x-data="{ 
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <button
                                ...
                                @click="
                                    ...
                                    valorSelecionado = {{ $feedback->valor_indicacao->id }};
                                    openEdicao = true;
                                ">
                                Editar
                            </button>
    ...
    <!-- Modal Edição -->
    <div x-show="openEdicao" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50" x-cloak>
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-96">
            <h2 class="text-lg text-white font-bold mb-4">Editar Feedback</h2>

            <form :action="'{{ route('feedback.update', '') }}/' + feedbackId" method="POST">
            @csrf
                @method('PUT')
                ...
                <div class="mt-4">
                    ...
                    <select
                        ...
                        <option value="">Selecione um valor</option>
                        @foreach ($valores as $valor)
                        <option x-bind:selected="valorSelecionado == {{ $valor->id }}" value="{{ $valor->id }}">{{ $valor->nome }}</option>
                        @endforeach
                    </select>
                </div>
                <!-- Campo Feedback -->
                ...
                <div class="mt-4 flex justify-between">
                    <button type="button" @click="openEdicao = false" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                        Fechar
                    </button>
                    <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                        Atualizar
